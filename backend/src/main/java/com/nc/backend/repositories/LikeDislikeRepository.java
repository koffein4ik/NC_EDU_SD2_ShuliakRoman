package com.nc.backend.repositories;

import com.nc.backend.model.LikedislikeEntity;
import com.nc.backend.model.PostsEntity;
import org.springframework.data.jpa.repository.Modifying;
import org.springframework.data.jpa.repository.Query;
import org.springframework.data.repository.PagingAndSortingRepository;
import org.springframework.data.repository.query.Param;
import org.springframework.transaction.annotation.Transactional;

import java.util.Optional;

public interface LikeDislikeRepository extends PagingAndSortingRepository<LikedislikeEntity, Integer> {
    @Query(value = "From LikedislikeEntity ld where ld.post.postId = :postId and ld.user.id = :userId")
    Optional<LikedislikeEntity> getLikeDislikeByPostIdAndUserId(@Param("postId") int postId,
                                                                @Param("userId") int userId);

    Optional<LikedislikeEntity> getByLikedilslikeId(Integer likeDislikeId);

        //    @Query(value = "From UserEntity usr where usr.nickname = :nickname and usr.password = :password")
//    Optional<UserEntity> authorize(@Param("nickname") String nickname,
//                                   @Param("password") String password);

    @Transactional
    @Modifying
    @Query(value = "Delete from LikedislikeEntity ld where ld.post.postId= :postId and ld.user.id = :userId")
    void removeLikeDislike(@Param("postId") Integer postId, @Param("userId") Integer userId);
    Iterable<LikedislikeEntity> findAllByPost(PostsEntity post);
}
